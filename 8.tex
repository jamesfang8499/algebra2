...
\item \textbf{解法1：}由(2)，3件中恰好有1件是次品的抽法有$ \comb{2}{1}\cdot  \comb{98}{2} $种，同理，抽出3件产品，恰好有2件是次品的抽法有 $\comb{2}{3}\cdot  \comb{91}{1} $种。因此抽出的3件中至少有一件是次品的抽法种数是
\[
\comb{2}{1}\cdot  \comb{98}{2}+\comb{2}{2}\cdot  \comb{98}{1}=9506+98=9604.\]

\textbf{解法2：}从100件产品中抽出3件，一共有$ \comb{100}{8} $种抽法，在这些抽法中，除掉抽出的3件都是合格品的抽法$ \comb{8}{8} $种，使得到抽出的3件中至少有1件是次品的抽法的种数，即
\[
\comb{100}{8}-\comb{98}{8}=161700-152096=9604.\]
...
这种解法得到的答案是$ \comb{2}{1}\cdot  \comb{9}{2}=9702$. 比前面解法多出98种。错误出在哪里？这里由于先取出的一件次品若是次品甲，在取剩下99件中的2件产品时，可能把次品乙和正品$a$取出，若先取出的一件次品是次品乙，而后取出的2件产品恰好是次品甲与正品$a$时，这两种取法显然是同一种取法，只是次品甲、乙取出次序不同。因而，这种解法实际上会出现重复的组合，种数当然比正确答案要多。
\end{rmk}
...
利用
\[\begin{split}
\comb{k}{0}=\comb{k+1}{0},\quad  \comb{k}{1}+\comb{k}{0}&=\comb{k+1}{1},\quad \cdots \quad \comb{k}{r+1}+\comb{k}{r}=\comb{k+1}{k+1},\\
...
\begin{solution}
设含$x^{3} $的项为第$( r+1 )$项，则
\[T{r+1}=\comb{6}{r}(2x)^{6-r}\left(\frac{1}{x^{2}}\right)^{r}=\comb{6}{2}\cdot  2^{6-r}\cdot  x^{6-3r}\]

$\therefore\quad 6-3r=3\quad \implies \quad r=1$

因此，$x^{3} $的系数是$\comb{6}{2}\cdot  2^{6-1}=192$.
\end{solution}

\begin{remark}
展开式中第$( r+1 )$项的二项式系数$\comb{n}{r}$ 与第$( r+1 )$项的系数不同。在上例中$ x^{3} $项是展开式的第2项，其二项式系是$\comb{1}{2}$, 而第二项（即含$ x^{3} $的项）的系数是$\comb{6}{2}\cdot  2^{5}=192$.    
\end{remark}
...
\textbf{解法2：}由于展开式中各项应是由$|x|$、$\frac{1}{|x|}$、$-2$所组成的因式之积。故常数项只可能由含$ |x|\cdot \frac{1}{|x|}\cdot (-2) $和$ (-2)^{3} $这样的两种形式的项构成。而$ |x|\cdot \frac{1}{|x|}\cdot (-2) $的系数是$\comb{1}{2}\comb{2}{1}(-2)$,  $(-2)^{3} $的系数是$\comb{3}{3}(-2)^{3}$. 所以常数项应为
...
中，设$ a=1$, $ b=-1$，得
\[(1-1)^{n}=\comb{n}{0}-\comb{n}{1}+C^{2}-\comb{n}{8}+\cdots +(-1)^{n}\comb{n}{n}\]
就有
\[
0=\left(\comb{n}{0}+\comb{n}{2}+\cdots \right)-\left(\comb{n}{1}+\comb{n}{8}+\cdots \right)\]

$\therefore\quad \comb{n}{0}+\comb{n}{3}+\cdots =\comb{n}{1}+\comb{n}{8}+\cdots$
...
\begin{example}
    求证： $\comb{8}{3}+\comb{4}{3}+\comb{5}{3}+\cdots +\comb{100}{3}=\comb{101}{4}$.
\end{example}

\begin{proof}
$\comb{8}{a}+\comb{4}{s}+\comb{5}{s}+\cdots +\comb{100}{3} $可看成$ (1+x)^{3}+(1+x)^{4} +(1+x)^{5}+\cdots +(1+x)^{100} $展开式中含$ x^{3} $项的系数。
